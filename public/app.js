...
const ALLOWED_SHORT_SOURCES = new Set(["AP", "NYT", "WSJ", "TOI", "BBC", "CNN"]);
const rejectedSourceLogCache = new Set();
const validateLeaderboardSource = (canonicalSource = "", rawSource = "") => {
  const canonical = String(canonicalSource || "").trim();
  const raw = String(rawSource || "").trim();
  if (!canonical) return { valid: false, reason: "empty" };
  if (/^(unknown|n\/a|na|null|undefined|source)$/i.test(canonical)) return { valid: false, reason: "placeholder" };
  if (/^["'`_-]+$/.test(canonical)) return { valid: false, reason: "symbol-only" };
  if (/^["'`_-]+$/.test(raw)) return { valid: false, reason: "raw-symbol-artifact" };
  if (!/[a-z]/i.test(canonical) && !ALLOWED_SHORT_SOURCES.has(canonical.toUpperCase())) return { valid: false, reason: "no-letters" };
  if (canonical.length <= 2 && !ALLOWED_SHORT_SOURCES.has(canonical.toUpperCase())) return { valid: false, reason: "too-short" };
  if (/[_]{2,}|["']{2,}/.test(canonical)) return { valid: false, reason: "quote-underscore-artifact" };
  return { valid: true, reason: "" };
};
const logRejectedSource = (rawSource, canonicalSource, reason) => {
  const key = `${String(rawSource || "").trim()}|${String(canonicalSource || "").trim()}|${reason}`;
  if (rejectedSourceLogCache.has(key)) return;
  rejectedSourceLogCache.add(key);
  console.info("[source-validation] rejected", {
    rawSource: String(rawSource || ""),
    canonicalSource: String(canonicalSource || ""),
    reason
  });
};
...
function computeLeaderboard(){
  ...
  if (splitRows.length){
    const arr = splitRows.map((row) => {
      ...
      const sourceValidation = validateLeaderboardSource(source, row.source || "");
      if (!sourceValidation.valid){
        logRejectedSource(row.source || "", source, sourceValidation.reason);
        return null;
      }
  ...
  getLeaderboardArticles().forEach(a => {
    ...
    const sourceValidation = validateLeaderboardSource(key, a.source || "");
    if (!sourceValidation.valid){
      logRejectedSource(a.source || "", key, sourceValidation.reason);
      return;
    }
    if (!key) return;
...
async function renderLeaderboard(){
  ...
  async function place(col, list){
    const safeList = list.filter((item) => {
      const sourceValidation = validateLeaderboardSource(item?.source || "", item?.source || "");
      if (!sourceValidation.valid){
        logRejectedSource(item?.source || "", item?.source || "", sourceValidation.reason);
      }
      return sourceValidation.valid;
    });
    const results = await Promise.all(safeList.map(s => (s.logo ? loadImage(s.logo) : Promise.resolve(false))));
    let idx = 0;
