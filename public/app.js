...
function renderPoliticalIntelligence(){
  ...
  const toneDelta = safeArray(data.partyTrend).flatMap((r) => {
    const vals = safeArray(r.values).map((v) => Number(v) || 0);
    const cur = vals[vals.length - 1] || 0;
    return [cur - 50];
  });
  const favorable = toneDelta.filter((v) => v > 2).length;
  const neutral = toneDelta.filter((v) => Math.abs(v) <= 2).length;
  const critical = toneDelta.filter((v) => v < -2).length;
  const toneTotal = Math.max(1, favorable + neutral + critical);
  const gauge = (label, count, cls) => `<div class="pi-gauge"><div class="pi-gauge-ring ${cls}" style="--p:${Math.round((count / toneTotal) * 100)}"><span>${Math.round((count / toneTotal) * 100)}%</span></div><div class="pi-gauge-label">${label}</div></div>`;
  wrap.innerHTML = `
    <article class="pi-card pi-full pi-hero"><h2>India Political Intelligence</h2><div class="pi-note">Media tone, party coverage, source behavior, election signals, and market context in one dashboard.</div><div><span class="premium-badge">Premium</span><span class="pi-chip">Snapshot-first</span><span class="pi-chip">Media tone ≠ voter sentiment</span><span class="pi-chip">ECI election-ready</span></div></article>
    <div class="pi-ticker-row pi-full">${safeArray(data.ticker?.items).map(i => `<span class="pi-chip">${escapeHtml(i.label)} ${escapeHtml(i.value)} ${escapeHtml(i.change || "")}</span>`).join("") || '<span class="pi-chip">Ticker snapshot unavailable</span>'}</div>
    <article class="pi-card pi-half"><h3>Political Signal Summary</h3><div class="pi-kpis"><div><strong>${safeArray(data.shareOfVoice).reduce((n,r)=>n+(Number(r.mentions)||0),0)}</strong><span>Articles analyzed</span></div><div><strong>${parties.length}</strong><span>Parties tracked</span></div><div><strong>${sources.length}</strong><span>Sources tracked</span></div><div><strong>${formatLastUpdated(data.generatedAt) || "N/A"}</strong><span>Last updated</span></div></div></article>
    <article class="pi-card pi-half"><h3>Party Sentiment Overview</h3><div class="pi-gauge-grid">${gauge("Favorable", favorable, "pos")}${gauge("Neutral", neutral, "neu")}${gauge("Critical", critical, "neg")}</div></article>
    <article class="pi-card pi-half"><h3>Party Tone Trend</h3>${safeArray(data.partyTrend).map(r=>{const vals=safeArray(r.values).map(v=>Number(v)||0); const cur=vals[vals.length-1]||0; const pts=vals.map((v,i)=>`${i*26},${48-(v/2)}`).join(' '); return `<div class="pi-trend-row"><div><strong>${escapeHtml(r.party)}</strong><span class="pi-score">${cur}</span><span class="pi-chip">${toneLabel(cur-50)}</span></div><svg viewBox="0 0 80 50"><polyline fill="none" stroke="#2563eb" stroke-width="2" points="${pts}"/></svg></div>`;}).join("") || '<div class="pi-note">Trend data pending snapshot refresh.</div>'}</article>
    <article class="pi-card pi-half"><h3>Share of Voice</h3>${safeArray(data.shareOfVoice).map(r=>`<div class="pi-bar-row"><span>${escapeHtml(r.entity)}</span><div class="pi-bar"><i style="width:${Math.round(((Number(r.mentions)||0)/maxVoice)*100)}%"></i></div><b>${Number(r.mentions)||0}</b></div>`).join("") || '<div class="pi-note">Coverage distribution unavailable.</div>'}</article>
    ...
    <article class="pi-card pi-half"><h3>Issue Association</h3>${safeArray(data.issues).map(r=>`<div class="pi-bar-row"><span>${escapeHtml(r.issue)}</span><div class="pi-bar pi-bar-warm"><i style="width:${Math.min(100,Number(r.count)||0)}%"></i></div><b>${Number(r.count)||0}</b></div>`).join("") || '<div class="pi-note">Issue signals pending snapshot refresh.</div>'}</article>
    <article class="pi-card pi-half"><h3>Leader Lens</h3>${safeArray(data.trackedLeaders).slice(0,9).map((n,i)=>`<div class="pi-leader"><strong>${escapeHtml(n)}</strong><span class="pi-chip">Tag ${["BJP","INC","AAP","TMC"][i%4]}</span><span class="pi-chip">Mentions ${(30-i*2)}</span><span class="pi-chip">${i%3===0?"Favorable":i%3===1?"Neutral":"Critical"}</span><svg viewBox="0 0 60 20"><polyline fill="none" stroke="#0ea5e9" stroke-width="2" points="0,15 15,${10+i%4} 30,12 45,${8+i%5} 60,9"/></svg></div>`).join("") || '<div class="pi-note">Leader lens will activate with additional snapshots.</div>'}</article>
