...
app.get("/icon-512.png", (req, res) => {
  ...
  res.sendFile(ICON_512_PATH, (err) => {
    if (err) {
...
/* markets */
const MARKET_FILE_PATH = path.join(__dirname, "public", "markets.json");
const MARKET_FALLBACK = [
  { symbol: "^NSEI", pretty: "NSE Nifty", price: 22000.0 },
  { symbol: "GC=F", pretty: "Gold", price: 2000.0 },
  { symbol: "CL=F", pretty: "Crude Oil", price: 62.24 },
  { symbol: "USDINR=X", pretty: "USD/INR", price: 91.56 }
];

const buildMarketFallbackPayload = () => {
  const now = Date.now();
  return {
    updatedAt: now,
    quotes: MARKET_FALLBACK.map((quote) => ({
      ...quote,
      change: null,
      changePercent: null,
      status: "unavailable",
...
const readMarketFile = () => {
  try {
    const raw = fs.readFileSync(MARKET_FILE_PATH, "utf8");
    return JSON.parse(raw);
  } catch {
    return null;
  }
