...
// Serve /public and index at /
const PUBLIC_DIR = path.join(__dirname, "public");
app.use(express.static(PUBLIC_DIR));
app.get("/", (_req, res) => res.sendFile(path.join(PUBLIC_DIR, "index.html")));

// Health (used for checks + ACME challenges reachability)
app.get("/healthz", (_req, res) => res.type("text").send("ok"));

// ----------------- NEWS PIPELINE -----------------
const parser = new Parser({ timeout: 15000 });

// Outlet lean (very light-touch)
const biasMap = {
  "pib.gov.in": "Center",
...
// India-only allowlist (keeps scope/legal clean)
const ALLOWED_DOMAINS = new Set(Object.keys(biasMap));

// Prefer these for the big/main slot
...
async function fetchFeed(src) {
  ...
  for (const it of items.slice(0, 30)) {
    ...
    // HEADLINE_ONLY → sentiment on headline only; OPEN/CC_* → headline + snippet
...
app.get("/api/news", async (_req, res) => {
  try {
    ...
    // Daily briefing: at least 5, mixed, no dupes
